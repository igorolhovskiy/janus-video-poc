	...
	janus.attach(
		{
			...
			success: function(pluginHandle) {
				...
				Janus.log("[SipVideoRoom][startVideo] Plugin attached! (" + sfutest.getPlugin() + ", id=" + sfutest.getId() + ")");
			...
			onmessage: function(msg, jsep) {
				...
				if(jsep) {
					...
					if(mystream && mystream.getAudioTracks() && mystream.getAudioTracks().length > 0 && !audio) {
						...
						toastr.warning("[SipVideoRoom][startVideo] Our audio stream has been rejected, viewers won't hear us");
					}
					var video = msg["video_codec"];
					if(mystream && mystream.getVideoTracks() && mystream.getVideoTracks().length > 0 && !video) {
						// Video has been rejected
						toastr.warning("[SipVideoRoom][startVideo] Our video stream has been rejected, viewers won't see us");
